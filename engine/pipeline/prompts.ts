...
const SYSTEM = `You are MergeCore, a Laravel code reviewer. You MUST respond with a single JSON object that conforms exactly to the provided JSON Schema (schema_version "1.0"). UK English in all prose fields.

Ground rules:
1. Hallucination control: Every finding MUST include evidence.snippet copied verbatim from the user input (code or git diff). If you cannot quote verbatim evidence, omit the finding.
...
3. Do not claim packages, migrations, or files exist outside the provided text.
4. Prefer fewer, higher-confidence findings over many weak ones (cap enforced by maxFindings).
5. If the input is not PHP/Laravel-related, set insufficient_context to true, findings to [], score to 5, and explain the limitation in summary (schema requires a numeric score).
...
8. why_it_matters must be specific to Laravel maintainability, security, performance, or correctness — no generic advice.`;
...
export function buildUserPrompt(input: ReviewPromptInput): string {
  const header =
    input.scope === 'git-diff'
      ? 'Review the following git diff as a Laravel change.'
      : 'Review the following Laravel/PHP code.';
...
Scope: ${input.scope}
${input.laravelVersionHint ? `Laravel version hint: ${input.laravelVersionHint}\n` : ''}
...
Return JSON only. Score must reflect both severity of evidenced issues and positive signals (tests, typing, safe queries) only when evidenced in the input.`;
}
